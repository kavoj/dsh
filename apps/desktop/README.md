...
The first application-menu command, **About DeepSeek Harness**, opens Electron's native About panel with the application icon, product name, and installed release version; a derived distribution shows its own product name and its upstream attribution there instead. The menu follows the Desktop shell locale. macOS reads the icon from its application bundle, so an unpackaged development launch displays Electron's icon; Windows receives the packaged PNG.
...
The packaged product identity defaults to `DeepSeek Harness`; the `suixing` client build profile selects the bundled SuiXing identity instead. `DSH_DESKTOP_PRODUCT_NAME` overrides the name, and `DSH_DESKTOP_ARTIFACT_SLUG` overrides the package-filename slug, which otherwise derives from that name. Any product name other than `DeepSeek Harness` also requires `DSH_DESKTOP_ICON_DIRECTORY` naming a directory that holds `icon-macos.png`, `icon.ico`, and `icon-windows.png`, so upstream artwork never ships under another product name. Every package command checks these fields before signing or downloading.

Any other product name additionally brands the native About panel and ships the repository `LICENSE` and `THIRD_PARTY_NOTICES.md` beside the packaged runtime, so a derived distribution states its DeepSeek Harness lineage, its MIT license, and its third-party notices. The upstream identity attributes nothing to itself and keeps its previous release bytes.
