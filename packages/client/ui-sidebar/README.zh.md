...
发行方可以通过本包持有的、面向注册方的服务 `ctx.sidebarCatalog`，把业务菜单作为数据发布出来。`register(group)` 提供一个 id、排序、已本地化的标题与可选辅助名称、该分组的条目，以及「查看全部」行打开的可选 root 作用域 `main` 面板键；`reportStatus(status, retry?)` 描述正在加载、加载失败或正在使用缓存副本的状态。外壳渲染已注册的内容——位置在 Workspaces 席位之上；若没有任何发行方发布过，则什么都不渲染，连外层元素也没有——并提供折叠状态、每组五行的最近使用预算、分组内搜索、查看全部跳转，以及四种加载呈现。折叠状态与最近使用按浏览器持久化在 `dsh.sidebar.catalog` 下；释放最后一个注册即释放该占用，区域不会比它背后的数据活得更久。
...
- **目录的交互词汇属于外壳**：注册方提供 id、排序、已本地化的文案、条目与面板键；折叠、五行预算、搜索与四种加载呈现不可配置。
...
**运行时不变式：** 不发布伴生入口。本包提供一个服务（`ctx.sidebarCatalog`）。目录注册及其状态随安装它们的注册方一起释放，唯一被持久化的状态是目录的折叠与最近使用切片；外壳没有需要与这些来源协调的独立导航状态。面板元数据仍是 Slot 注册表与 locale 的只读呈现投影，没有独立写入 API：注册表负责条目身份与资源释放，本包的装配测试在注册和 locale 通知完成后断言该投影。
